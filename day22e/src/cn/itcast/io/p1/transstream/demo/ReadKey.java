...
//			在存储之前需要判断是否是换行标记 ,因为换行标记不存储。 
			if(ch=='\r')//【\r \n 是连在一起的，】
				continue;//【继续下一次循环，不执行else 里面的语句】
			if(ch=='\n'){
...
//			System.out.println(ch);
	...
	public static void readKey() throws IOException {
		...
		InputStream in = System.in;//【InputStream 字节输入流 。System 类中  的 静态属性in】
		
		int ch = in.read();//阻塞式方法。		
		System.out.println(ch);
		int ch1 = in.read();//阻塞式方法。		read是来读取数据的，没数据，会一直等
		System.out.println(ch1);
		int ch2 = in.read();//阻塞式方法。		
		System.out.println(ch2);
		
//		in.close();【不要关，默认的输入、输出设备不需要关，关了，下面不能再创建】
