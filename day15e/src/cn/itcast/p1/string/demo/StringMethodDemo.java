...
public class StringMethodDemo {
	...
	public static void main(String[] args) {
		...
		/*
		 * 按照面向对象的思想对字符串进行功能分类。【实际开发的时候，直接看高级编译器里面的提示，一般不查 api】
		 * "abcd"
		 * 
		 * 1,获取：
		 * 1.1 获取字符串中字符的个数(长度).
		 ...
		 * 1.4 获取字符串中一部分字符串。也叫子串.（肯定不是静态 的，因为，要操作对象）
		 ...
		 * 			String[]  split(String regex):涉及到正则表达式.【传递的是一个规则】
		 * 		2.2 将字符串变成字符数组。
		 * 			char[] toCharArray();
		 * 		2.3 将字符串变成字节数组。
		 * 			byte[] getBytes();
		 ...
		 * 		2.6 将字符串两端的空格去除。【用户输入 ，最后，可能输入多余空格】
		 * 			String trim();
		 * 		2.7 将字符串进行连接 。【对+ 进行封装】
...
//		System.out.println(""+4+1);
	...
	private static void stringMethodDemo_1() {
		...
		System.out.println("index:"+s.indexOf('b'));//0//-1 我们可以根据-1，来判断该字符或者字符串是否存在。
